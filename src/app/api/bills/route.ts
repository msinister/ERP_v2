...
import { BillSource, BillStatus } from '@/generated/tenant';
import { createBillInputSchema } from '@/lib/validation/ap';
import { createBill, listBills } from '@/server/services/bills';
...
export async function GET(req: Request) {
  try {
    ...
    const list = await listBills(db, {
      vendorId,
      status,
      source,
      billDateFrom: fromParam ? new Date(fromParam) : undefined,
      billDateTo: toParam ? new Date(toParam) : undefined,
      q,
      skip,
      take,
    });
