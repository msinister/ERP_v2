...
export function AdjustmentsPagination({
  total,
  skip,
  take,
}: {
  ...
  function goToPage(page: number) {
    ...
    router.push(`/inventory-adjustments?${next.toString()}`);
  }
...
function countLabel(from: number, to: number, total: number): string {
  if (total === 0) return 'No adjustments';
...
function computePageWindow(
  ...
  const showLeft = currentPage - 1 > 2;
  const showRight = currentPage + 1 < pageCount - 1;
  result.push(1);
  if (showLeft) result.push('ellipsis');
  const start = Math.max(2, currentPage - 1);
  const end = Math.min(pageCount - 1, currentPage + 1);
  for (let p = start; p <= end; p++) result.push(p);
  if (showRight) result.push('ellipsis');
