...
// URL-driven pagination. Owns the `skip` param; reads `take` to know
// page size. All other params are preserved verbatim. The host page
// computes total from listCustomersPaged and passes it in.
...
export function CustomersPagination({
  ...
  if (total <= take && skip === 0) {
    // Single-page result — show only the count, no page controls.
    return (
...
function countLabel(from: number, to: number, total: number): string {
  if (total === 0) return 'No customers';
  if (from === to) return `${formatCount(from)} of ${formatCount(total)}`;
  return `${formatCount(from)}–${formatCount(to)} of ${formatCount(total)}`;
}

// Returns the list of buttons to render: page numbers interspersed
// with 'ellipsis' markers. Algorithm: always show first + last; show
// a window around current; collapse the rest.
